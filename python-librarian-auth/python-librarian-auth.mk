################################################################################
#
# python-librarian-auth
#
################################################################################

PYTHON_LIBRARIAN_AUTH_VERSION = 0306b940bcb128ff96edfae62688c6f8497b6546
PYTHON_LIBRARIAN_AUTH_SITE = $(call github,Outernet-Project,librarian-auth,$(PYTHON_LIBRARIAN_AUTH_VERSION))
PYTHON_LIBRARIAN_AUTH_LICENSE = GPLv3+
PYTHON_LIBRARIAN_AUTH_LICENSE_FILES = COPYING
PYTHON_LIBRARIAN_AUTH_DEPENDENCIES = python-librarian-core
PYTHON_LIBRARIAN_AUTH_SETUP_TYPE = setuptools

$(eval $(python-package))
